...
# experiment settings
use1HOT = 1 # N.b> this iverwrites output catagories
...
lenOfInput = 500
inSize=lenOfInput
noOfTestData = 5000 # THIS EXPERIMENT USES NO TEST DATAS!!!
doKmeans =0
noOfEpochs= 60000
...
try:
    opts, args = getopt.getopt(sys.argv[1:],"n:H:D:d:T:E:F:ceg:",
                               ["name=","HLN=","decay=","dropout=","traindata=","epoch=","function=a,""cpu","earlystopper","gaussian="])
    ...
    print("{0}: [-n|--name=<name>] [-H|--HLN=<neuron count>] [-D|--decay=<decay probability>]"
          ...
          "[-c|--cpu] [-e|--earlystopper] [-g|--gaussian]".format(sys.argv[0]))
    sys.exit(1)
for opt,arg in opts:
    if opt in ('-n', '--name'):
        name = arg
...
XVal = code.make_prototyped_random_codes(M=int(noOfTrainData*0.3), n=lenOfInput, p=noOfPrototypes, weight=[weightOfR],
                                      k=2,symbolList=None, verbose=True, decay_templates=decay)
XTest = code.make_prototyped_random_codes(M=noOfTestData, n=lenOfInput, p=noOfPrototypes, weight=[weightOfR],
                                      k=2,symbolList=None, verbose=True, decay_templates=decay)
...
if use1HOT==1:
    ...
    T = np.zeros([noOfTrainData, sizeOfOutput])
    t = code.make_random_codes(M=noOfOutputs, n=sizeOfOutput, weight=[0.1], k=2, verbose=True)
    print('You are using distributed output encoding')
    print(t)
n = 0
for p in range(noOfPrototypes):
    for z in range(noOfTrainData//noOfPrototypes):
        T[n, :] = t[p]
        n = n + 1

do_shuffle=0
if do_shuffle:
    # shuffle the vectors to make the NN memorize
    n = 0
    for z in range(noOfTrainData):
        p=z%noOfPrototypes
        T[n, :] = t[p]
        n = n + 1
        #print(n,p)
...
#### so fucking hacky! #############TEST SET #########################
## this makes our output vectors - relies on C being sorted (which it is)
if use1HOT==1:
    # we use the no of catagories and overwrite the output vectors size
    TTest = np.zeros([noOfTestData, sizeOfOutput])
    tTest = d.make_1HOT(sizeOfOutput)
    #print('You are using 1-HOT output encoding')
    #print(t)
else:
    TTest = np.zeros([noOfTestData, sizeOfOutput])
    tTest = code.make_random_codes(M=noOfOutputs, n=sizeOfOutput, weight=[0.1], k=2, verbose=True)
    #print('You are using distributed output encoding')
    #print(tTest)
n = 0
for p in range(noOfPrototypes):
    for z in range(noOfTestData//noOfPrototypes):
        TTest[n, :] = t[p]
        n = n + 1
...
if use1HOT==1:
    # we use the no of catagories and overwrite the output vectors size
    TVal = np.zeros([int(noOfTrainData*0.3), sizeOfOutput])
    tVal = d.make_1HOT(sizeOfOutput)
    print('You are using 1-HOT output encoding')
    print(t)
else:
    TVal = np.zeros([int(noOfTrainData*0.3), sizeOfOutput])
    tVal = code.make_random_codes(M=noOfOutputs, n=sizeOfOutput, weight=[0.1], k=2, verbose=True)
    print('You are using distributed output encoding')
    print(t)
n = 0
for p in range(noOfPrototypes):
    for z in range(int(noOfTrainData*0.3)//noOfPrototypes):
        TVal[n, :] = tVal[p]
        n = n + 1
...
if doDropout:
    ...
    model = Sequential()
    model.add(Dense(HLN, kernel_initializer='uniform', input_dim=inSize))
    model.add(Activation('sigmoid'))
    #model.add(Dropout(0.2))
    model.add(Dense(250, kernel_initializer='uniform'))
    model.add(Activation('sigmoid'))
...
if doEarlyStopper:
    pass
    #callback_list.append(keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=250, verbose=1, mode='min'))
    #callback_list.append(TerminateOnBaseline(monitor='acc', baseline=1.0))
    #callback_list.append(TerminateOnBaseline(monitor='val_acc', baseline=1.0))
##################################################################
# 2. Train the model
##################################################################
...
fit = model.fit(X, T, epochs=noOfEpochs, batch_size=32,
                shuffle=True, callbacks=callback_list)

# normal setting!
#fit = model.fit(X, T, validation_data=(XVal,TVal), epochs=noOfEpochs, batch_size=32,
#                shuffle=True, callbacks=callback_list)
